...
public class ContainerNameHttpHeaderFilterConfiguration {
  ...
  public ContainerNameHttpHeaderFilter containerNameHttpHeaderFilter() {
    LOGGER.trace(
        "Configuration: header '{}', order '{}', hostname '{}'.",
        headerName,
        filterOrder,
        hostname);
   ...
   * @return the {@link #containerNameHttpHeaderFilter()} bean, wrapped in a {@link
   *     ContainerNameHttpHeaderFilter}
   */
  @Bean
  @ConditionalOnProperty(prefix = CONFIG_PREFIX, name = "enabled", matchIfMissing = true)
  public FilterRegistrationBean<ContainerNameHttpHeaderFilter>
      containerNameHttpHeaderFilterRegistration(ContainerNameHttpHeaderFilter filter) {
