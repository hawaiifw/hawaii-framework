 ...
 * <ol>
 * <li>Executors</li>
 * <li>Systems, containing Calls</li>
 * </ol>
 * <p>
 * From each of the executor configurations,
 * a {@link org.springframework.scheduling.concurrent.ThreadPoolTaskExecutor} will be created. The default executor is the fallback
 * for any task not specifically configured to an executor.
 * <p>
 * A system is a group of tasks, assumed to belong to a single system, for example a backend system. Both a system and a call can be
 * assigned to an executor, where the task configuration takes precedence.
 * <p>
...
public class ExecutorConfigurationProperties {
     ...
     * The default queue.
     * <p>
     * Will be used to set the default executor on the Spring async framework.
     */
     ...
     * The default timeout.
     * <p>
     * This is the fallback in case no timeout is specified on either system or call.
     */
     ...
     */
    public void setAsyncTimeoutExecutorPoolSize(final Integer asyncTimeoutExecutorPoolSize) {
     ...
     */
    public void setExecutors(final List<ExecutorProperties> executors) {
     ...
     */
    public void setSystems(final List<SystemProperties> systems) {
     ...
     * <p>
     * The task name must be formatted as <code>{system}.{task}</code>. If a specific timeout is configured for the task, it is
    ...
    public Integer getTaskTimeout(final String taskName) {
        final String[] parts = taskName.split(SEPARATOR);

        final SystemProperties systemProperties = getSystemPropertiesForName(parts[0]);

        Integer timeout = null;

        if (systemProperties != null) {
            final TaskProperties taskProperties = systemProperties.getTaskPropertiesForName(parts[1]);
