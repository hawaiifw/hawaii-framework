...
public class RecipeControllerTests extends AbstractMockMvcTest {
    ...
    public void listShouldReturnRecipes() throws Exception {
        mockMvc.perform(get(RECIPES_LIST_PATH)).andExpect(status().isOk())
                ...
                .andExpect(
                        jsonPath("$[0].ingredients[1].description", is("teaspoon ground ginger")))
                ...
                .andExpect(
                        jsonPath("$[0].ingredients[3].description", is("tablespoon onion powder")))
                .andExpect(jsonPath("$[0].ingredients[4].quantity", is(2.0)))
                .andExpect(
                        jsonPath("$[0].ingredients[4].description", is("tablespoons garlic salt")))
                .andExpect(jsonPath("$[0].ingredients[5].quantity", is(3.0)))
                .andExpect(jsonPath("$[0].ingredients[5].description",
                        is("tablespoons cider vinegar")))
                ...
                .andExpect(jsonPath("$[0].ingredients[8].description",
                        is("(20 ounce) can crushed pineapple with juice")))
                .andExpect(jsonPath("$[0].ingredients[9].quantity", is(0.25)))
                .andExpect(
                        jsonPath("$[0].ingredients[9].description", is("cup packed brown sugar")))
                ...
                .andExpect(
                        jsonPath("$[1].ingredients[0].description", is("cups all-purpose flour")))
                ...
                .andExpect(
                        jsonPath("$[1].ingredients[4].description", is("teaspoon ground cinnamon")))
                ...
                .andExpect(jsonPath("$[1].ingredients[8].description",
                        is("cups mashed very ripe banana")))
                .andExpect(jsonPath("$[1].ingredients[9].quantity", is(1.0)))
                .andExpect(jsonPath("$[1].ingredients[9].description",
                        is("(8 ounce) can crushed pineapple, drained")))
                .andExpect(jsonPath("$[1].ingredients[10].quantity", is(2.0)))
                .andExpect(jsonPath("$[1].ingredients[10].description",
                        is("teaspoons vanilla extract")))
    ...
    public void getShouldReturnRecipe() throws Exception {
        mockMvc.perform(get(RECIPES_GET_PATH, 1)).andExpect(status().isOk())
                ...
                .andExpect(
                        jsonPath("$.ingredients[5].description", is("tablespoons cider vinegar")))
                ...
                .andExpect(jsonPath("$.ingredients[8].description",
                        is("(20 ounce) can crushed pineapple with juice")))
                .andExpect(jsonPath("$.ingredients[9].quantity", is(0.25)))
                .andExpect(jsonPath("$.ingredients[9].description", is("cup packed brown sugar")))
                .andExpect(jsonPath("$.instructions", is(
                        "<p>Preheat oven to 400 degrees F (200 degrees C).</p><p>Arrange chicken pieces in a single layer in a well greased 9x13 inch baking dish. In a small bowl mix together the ginger, paprika, onion powder and garlic salt. Add the vinegar and mix well. Divide this mixture. Brush 1/2 over the chicken pieces and bake in the preheated oven for 15 minutes.</p><p>Turn the chicken pieces, baste with the remaining 1/2 of the vinegar mixture and bake for 15 minutes longer. Meanwhile, in a medium bowl combine the ketchup, soy sauce, pineapple and brown sugar. When chicken baking time is up, spoon the pineapple/soy mixture over the chicken. Bake for another 30 minutes. Serve while still hot.</p>")));
    }
}
