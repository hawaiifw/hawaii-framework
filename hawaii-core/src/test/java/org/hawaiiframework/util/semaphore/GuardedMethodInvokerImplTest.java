...
public class GuardedMethodInvokerImplTest {
    ...
    public void methodsCannotBeInvokedInParallel() throws InterruptedException {
        ...
        final Thread thread1 = new Thread(() -> invoker.invokeIfNotActive(() -> {
            startLatch.countDown();
            toggleAndSleep(method1Invoked, 30L);
            stopLatch.countDown();
        }));
        final Thread thread2 = new Thread(() -> invoker.invokeIfNotActive(() -> {
    ...
    public void multipleGuardedInvokersCanBeInvokedInParallel() throws InterruptedException {
        ...
        final Thread thread2 = new Thread(() -> invoker2.invokeIfNotActive(() -> {
            toggleAndSleep(method2Invoked, 30L);
            stopLatch.countDown();
        }));
        thread1.start();
    ...
    private void toggleAndSleep(AtomicBoolean method, long sleep) {
