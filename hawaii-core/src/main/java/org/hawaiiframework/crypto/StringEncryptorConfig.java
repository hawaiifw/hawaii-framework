 ...
 *
 * <p>This is moved to a separate class to make sure the StringEncryptor, which we're overriding
 * here to use the {@link HawaiiStringEncryptor} is created before any encrypted properties are
 * accessed. Otherwise, Jasypt will fall back to its default {@link StringEncryptor}.
 */
@Configuration
public class StringEncryptorConfig {
   ...
   *
   * <p>The environment variable identified by encryptionInit contains the key and the init vector.
   *
  ...
  public StringEncryptor stringEncryptor() {
    ...
    String encryptionInit = System.getenv(encryptionInitProperty);
    Assert.hasLength(
        encryptionInit,
        String.format(
            "Encryption cannot be initialized without '%s' environment variable.",
            encryptionInitProperty));

    String[] parts = splitEncryptionInitializationString(encryptionInit);
    return createStringEncryptor(parts);
  }

  private static StringEncryptor createStringEncryptor(String... parts) {
    return new HawaiiStringEncryptor(parts[0], parts[1]);
  }

  private static String[] splitEncryptionInitializationString(String encryptionInit) {
    String[] parts = encryptionInit.split(":");
    if (parts.length != NUM_PARTS) {
      throw new IllegalArgumentException(
          String.format(
              "The environment variable '%s' must consist of two parts, separated by a colon.",
              encryptionInit));
    }
    return parts;
  }
